...
class DiskStorage<T: DataTransformable>: @unchecked Sendable {
    ...
    func store(value: T, forKey key: String, expiration: StorageExpiration? = nil ) async throws {
        ...
        /// Disk에 대한 접근이 패키지 외부에서 동시에 이루어질 경우, 동일한 위치에 다른 데이터가 덮어씌워지는 data race 상황이 됩니다. 이를 방지하고자, 기존 Kingfisher에서는 DispatchQueue를 통해  직렬화 큐를 구현한 후, store(Write), value(Read)를 직렬화 큐에 전송하여 순차적인 실행이 보장되게 하였습니다.
    ...
    func value(
        ...
        return try await serialActor.run { () -> T? in
            ...
            if extendingExpiration != .none {
                ...
                case .expirationTime(let storageExpiration):
...
extension DiskStorage {
    ...
    /// 사전에 패키지에서 설정된 Config 구조체를 통해 파일명을 해시화하기로 설정했는지 여부, 임의로 전달된 접미사 단어 유무에 따라 캐시될때 저장될 파일명을 변환하여 반환해줍니다.
    ...
    private func prepareDirectory() throws {
        /// config에 custom fileManager를 주입할 수 있기 때문에, 여기서 .default를 접근하지 않고 Config 내부 fileManager를 접근합니다.
