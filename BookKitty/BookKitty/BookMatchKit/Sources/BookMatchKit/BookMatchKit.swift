...
public final class BookMatchKit: BookMatchable {
    ...
    public func matchBook(_ rawData: [[String]], image: UIImage) -> Single<BookItem?> {
        ...
        /// 클로저 내부에서 두가지 경우 just([]), fetchSearchResults()의 Single을 반환하고 있어 정확한 타입을 추론하기 어려워 함 -> 타입 모호성 에러 발생 -> 반환 타입 명시적으로 지정 필요
        ...
        return Single<[BookItem]>.create { single in
            ...
            func processNextQuery() {
                ...
                Single<Void>.just(())
                    ...
                    /// `flatMap`: Single 시퀀스의 각 항목을 다른 Single로 변환하고, 그 결과들을 하나의 Single로 평탄화(flatten)하는 연산자
                    /// Timer Single에서 searchBooks api의 결과값에 대한 Single로 스트림을 전환
                    .flatMap { _ in
                        self.apiClient.searchBooks(query: currentQuery, limit: 10) // Single<[BookItem]>을 반환
                    }
                    .subscribe(
            ...
            /// - Disposable을 반환함으로써, 구독이 해제될 때 필요한 정리 작업을 수행할 수 있습니다. 이는 구독이 해제될 때 진행중인 작업을 취소하고 리소스 정리가 가능하게 합니다
...
/// `.asSingle`: 최종 `Observable`이 방출한 여러 값들 중에 `마지막으로 방출된 값`만 Single의 `success` 이벤트로 전달됨.
